...
class MovieAdmin(admin.ModelAdmin):
    list_display = ('title', 'release_year', 'rate', 'average_rating',
                    'related_countires', 'related_languages', 'related_genres', 'choosen_home_page', 'trend')
    list_editable = ('choosen_home_page', 'trend')
    search_fields = ('title',)
    list_filter = ('choosen_home_page', 'trend', 'countries', 'languages', 'genres', 'release_year')
...
class SeriesAdmin(admin.ModelAdmin):
    list_display = ('title', 'release_year', 'rate',
                    'average_rating', 'choosen_home_page', 'trend', 'related_countires')
    list_editable = ('choosen_home_page', 'trend')
    search_fields = ('title',)
    list_filter = ('choosen_home_page', 'trend', 'countries', 'languages', 'genres', 'release_year')
    filter_horizontal = ('countries', 'languages', 'genres')
...
class DownloadFileAdmin(admin.ModelAdmin):
    ...
    def movie_or_series(self, obj):
        return obj.movie.title if obj.movie else obj.episode if obj.episode else "None"
...
class WeeklyScheduleAdmin(admin.ModelAdmin):
    list_display = ('series', 'day_of_week', 'air_time', 'is_active')
    list_filter = ('day_of_week', 'is_active', 'series')
    search_fields = ('series__title', 'notes')
    autocomplete_fields = ('series',)
    list_editable = ('is_active', 'air_time')
    list_per_page = 25
    ...
    fieldsets = (
        ...
        ('Broadcast Information', {
            'fields': ('day_of_week', 'air_time', 'is_active')
        }),
        ('Notes', {
            'fields': ('notes',)
        }),
...
class ShortDescriptionAdmin(admin.ModelAdmin):
    list_display = ('series', 'is_active', 'created_at', 'updated_at')
    list_filter = ('is_active', 'series')
    search_fields = ('series__title', 'description')
    autocomplete_fields = ('series',)
